...
(define (view:create tbl #!key
  ...
  (let* ((view-columns (if include-deleted
         ...
         (sorted-rows (if sort-column
                          ...
                          filtered-rows)))
    (%make-delectus-table tbl
                          (%make-column-sequence (list->vector view-columns))
...
(define (view:create tbl #!key (description (view:null-description)))
  (let* ((view-columns (if include-deleted
         ...
         (view-column-indexes (map (partial table:column-index tbl)
                                   (map column:label view-columns)))
         (live-rows (if (view-description:include-deleted? description)
                        (%table-select-rows tbl (always #t))
                        (%table-select-rows tbl (complement row:deleted?))))
         (view-rows (map (lambda (r)(row:make-with-row-entries r view-column-indexes))
                         live-rows))
         (filtered-rows (if (view-description:filter-text description)
                            (filter (lambda (row)(row:match-text? row (view-description:filter-text description)))
                                    view-rows)
                            view-rows))
         (sorted-rows (if (view-description:sort-column description)
                          (sort filtered-rows (%make-row-comparator tbl
