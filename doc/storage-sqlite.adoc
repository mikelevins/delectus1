...
An `identity` starts as a *time-first* UUID string--that is, a version 4 UUID in which the time fields are printed first. This arrangement enables databases--like SQLite--to index and sort these IDs efficiently, and to sort them in time order.
