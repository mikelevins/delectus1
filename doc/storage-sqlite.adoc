...
Two different copies of the same list may have different `origin` values. The `origin` records the identity of the node that maintains this copy of the list, not the identity of the node where it was first created. That means that Delectus can distinguish two different copies of the same list that come from two different nodes. The `id` identifies the list; the `origin` identifies which copy this is.
|`"format"` |`TEXT` | The current version of the Delectus data format used in this file.
|`"next_revision"` |`INTEGER` | The revision number that will be assigned to the next op
