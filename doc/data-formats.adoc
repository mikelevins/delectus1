...
| `metadata` | metadata | A JSON object giving op-specific metadata in addition to the common columns `type`, `opid`, `origin`, and `timestamp`. See the `<<metadata-definition>>` definition, below.
...
`metadata` objects appear in all ops, where they specify attributes of the op in addition to those in the common columns, `type`, `opid`, `origin`, and `timestamp`.
...
| `type` | `"sync"`,`"listname"`,`"columns"`,`"item"` | Identifies the type of op.
...
The columns `type`, `opid`, `origin`, `timestamp`, and `metadata` are always the same in every Delectus list file.
