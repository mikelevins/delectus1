...
The contents of each `*_label_*` field in the `"columns"` table is a JSON *column definition* object with the following format:
...
| `*order*` | `double` | A decimal number giving the desired order of presentation for the column. By default, columns are numbered 100.0, 200.0, and so on.
...
What you can do instead is ask Delectus to make a new concurrency-disabled copy of the listfile. The new file gets a new unique ID. The contents of its `"items"` table are taken from the old file's temporary `"latest_items"` table. Because the new file's support for concurrent edits is off by default, the `"origin"` column is not copied, and is therefore lost from the new file.

You can then enable concurrent edits for the new file if you like. Delectus treats the new file and any concurrent copies as entirely separate from the original file. When it adds the `"origin"` columns to the tables, it fills them with the unique origin of the current Delectus process.
