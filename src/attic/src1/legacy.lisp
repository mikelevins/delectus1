...
;;;   ... 
;;;
;;; The legacy code reads a file in either format and converts the
;;; data to Delectus 2 data suitable for storage in a SQLite file. It
;;; is an error if the number of fields in any row differs from the
;;; number of labels.
...
;;; the file at PATH, returning the pathname of the created file. The
;;; file at PATH must be in lecter's SEXP export format.
  ...
  (let* ((outpath (or outpath
    ...
    (with-open-file (in path :direction :input)
      (let ((sentinel nil))
        ...
        (assert (eql :DELECTUS sentinel)()
                "File format error: Expected :DELECTUS but found ~S" sentinel)
        ;; remove the :SEXP sentinel
        (setf sentinel (read in))
        (assert (eql :SEXP sentinel)()
                "File format error: Expected :SEXP but found ~S" sentinel)
        ;; remove the :COLUMNS sentinel
        (setf sentinel (read in))
        (assert (eql :COLUMNS sentinel)()
                "File format error: Expected :COLUMNS but found ~S" sentinel)
        ;; 2. read columns
        (let ((columns (read in)))
          ;; remove the :ROWS sentinel
          (setf sentinel (read in))
          (assert (eql :ROWS sentinel)()
                  "File format error: Expected :ROWS but found ~S" sentinel)
          ...
          (let* ((column-labels columns))
            ...
            (with-open-database (db outpath)
              (with-transaction db
                ...
                (loop
                   for row = (read in nil nil nil)
                   then (read in nil nil nil)
                   while row
                   do (let* ((fields (mapcar #'sexp-val->delectus-val row))
                             (insert-sql (format nil
...
;;; Creates a new Delectus file containing the converted contents of
;;; the file at PATH, returning the pathname of the created file. The
;;; file at PATH must be in lecter's CSV export format.
        ...
        (let ((column-labels (read-csv-line in)))
          ...
          (with-open-database (db outpath)
            (with-transaction db
              ;; read and insert rows
              (loop
                 for row = (read-csv-line in)
                 then (read-csv-line in)
                 while row
...
;;; (defparameter $junior-infile "/Users/mikel/Desktop/junior-movies.csv")
;;; (defparameter $junior-outfile "/Users/mikel/Desktop/junior-movies.delectus2")
;;; (time (convert-delectus-csv-file $junior-infile $junior-outfile))
;;;
...
;;; (defparameter $zip-infile "/Users/mikel/Workshop/src/delectus/test-data/zip_codes_states.csv")
;;; (defparameter $zip-outfile "/Users/mikel/Desktop/zip_codes_states.delectus2")
;;; (time (convert-delectus-csv-file $zip-infile $zip-outfile))
