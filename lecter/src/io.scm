...
(define (make-couch-list columns)
  (let* ((uuid (make-uuid))
         (listid (string-append "list:" uuid)))
    `(("_id" . ,listid)
      ("type" . "List")
      ("columns" . ,(map identity columns)))))
...
(define (write-couchdb path)
  (let* ((data (delectus->lisp path))
         ...
         (rows-tail (member 'ROWS data))
         (rows (if rows-tail (cadr rows-tail) '()))
         (listobj (make-couch-list columns))
         (rows (map (lambda (row) (make-couch-listitem listobj columns row))
                    rows)))
    (newline)
    (write listobj)
    (newline)
